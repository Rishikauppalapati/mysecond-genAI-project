"""
Phase 4: FundWise AI Chatbot - Final Logic Fix
CSV-Based Source Mapping with Multi-Fund Support
"""
...
def load_document_links():
    ...
    csv_path = r"c:\Users\RISHIKA UPPALAPATI\Downloads\mutual funds - Sheet1.csv"
    ...
    try:
        ...
        # SID row (index 9) - columns 0,3,6,9 contain URLs
        if len(df) > 9:
            row = df.iloc[9]
            ...
            # Leaflet URLs are also in row 9 at same positions (they contain 'leaflet' or 'factsheets')
            for fund, col_idx in [("Axis Large Cap Fund", 0), ("Axis Small Cap Fund", 3), 
                ...
                if val and ('leaflet' in val.lower() or 'factsheets' in val.lower()):
...
def load_css():
    ...
    .msg-avatar.user {background: #ff5722; color: white;}
    .msg-avatar.bot {background: #ff9800; color: white;}
    .msg-content {flex: 1; background: white; padding: 0.75rem 1rem; border-radius: 12px; 
                  border: 1px solid #e0e0e0; font-size: 0.9rem; line-height: 1.5;}
    .msg-content.user {background: #e3f2fd;}
    .msg-source {margin-top: 0.5rem; padding-top: 0.5rem; border-top: 1px solid #e0e0e0;}
    .msg-source a {color: #2196f3; text-decoration: none; font-size: 0.8rem;}
...
def detect_funds_in_query(query: str) -> list:
    ...
    # Check for specific fund names
    for fund in ALL_FUNDS:
        if fund.lower() in query_lower:
            detected.append(fund)
    
    # Check for partial matches if no full matches
    if not detected:
        if 'large cap' in query_lower: detected.append("Axis Large Cap Fund")
        elif 'small cap' in query_lower: detected.append("Axis Small Cap Fund")
        elif 'nifty 500' in query_lower: detected.append("Axis Nifty 500 Index Fund")
        elif 'elss' in query_lower or 'tax saver' in query_lower: detected.append("Axis ELSS Tax Saver")
    
    # Check for "all funds" or similar
...
def detect_data_type(query: str) -> str:
    ...
    if 'nav' in query_lower: return 'NAV'
    if 'expense' in query_lower: return 'expense_ratio'
    if 'sip' in query_lower: return 'MIN SIP'
    if 'exit' in query_lower or 'load' in query_lower: return 'exit_load'
    if 'risk' in query_lower: return 'riskometer'
    if 'benchmark' in query_lower: return 'benchmark'
    if 'kim' in query_lower: return 'KIM'
    if 'sid' in query_lower: return 'SID'
    if 'leaflet' in query_lower: return 'Leaflet'
    return 'NAV'
...
def get_document_response(fund: str, doc_type: str, docs: dict) -> dict:
    ...
    return {
        'answer': f"Sorry, {doc_type} document for {fund} is not available.",
        'source_url': None,
        'source_name': None,
        'last_updated': datetime.now().strftime("%Y-%m-%d")
    }
...
def process_query(query: str, backend, csv_manager, docs, fund_data) -> list:
    ...
    if not funds[0] and not is_document_query(query):
        # Check if query is asking for NAV, SIP, expense, etc.
    ...
    for fund in funds:
        ...
        if explicit_value:
            # Format the answer with explicit value
        ...
        if last_updated:
            try:
                # Format: "17 Mar 2026 12:00:00" -> "17 Mar 2026"
